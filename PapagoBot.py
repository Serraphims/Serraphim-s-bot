...
#discord bot tokken
token = 'NzEzNDMzMTM5NTI3MTU1NzYy.XsgWWA.u8vY1k3Ym8zzZUpQtPNMZQt15uY'
#Naver Open API application ID
client_id = "kANsx4cooMIXRywIWJt4"
#Naver Open API application token
client_secret = "TZbPwoQhMt"

client = discord.Client()
@client.event # Use these decorator to register an event.
async def on_ready(): # on_ready() event : when the bot has finised logging in and setting things up
    await client.change_presence(status=discord.Status.online, activity=discord.Game("세라핌은 열일"))
        ...
        try:
            if len(trsText) == 1:
                ...
                if (responsedCode == 200):
                    ...
                    await message.channel.send("세라핌 번역완료", embed=embed)
                else:
        ...
        try:
            if len(trsText) == 1:
                ...
                if (responsedCode == 200):
                    ...
                    embed.set_footer(text="Service provided by Hoplin. API provided by Naver Open API",
                                     icon_url='https://avatars2.githubusercontent.com/u/45956041?s=460&u=1caf3b112111cbd9849a2b95a88c3a8f3a15ecfa&v=4')
                    await message.channel.send("Serraphim translated perfectly", embed=embed)
                else:
                    await message.channel.send("Error Code : " + responsedCode)
        except HTTPError as e:
            await message.channel.send("Translate Failed. HTTPError Occured.")
